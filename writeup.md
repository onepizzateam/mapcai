...
The *generator's* region bucket (`regionHint`, used for plates/model pools) and the *display layer's* country label (`lib/geo/resolveCountry.ts`, real point-in-polygon lookup against `world-countries.topo.json`) are two independent classifications of the same bin. They agree in the vast majority of cases; at the margins, a bin can be sampled into one bucket while its lat/lng resolves to an adjacent country's borders. Root cause and fix are both simple: derive `regionHint` from the same `resolveCountry()` call the sidebar already uses, so there's exactly one source of truth. Listed in [Known limitations](#known-limitations).
...
1. **Live SSE feed is idle on the deployed demo.** The wiring is real and the tests cover it — but sustaining a live writer against 25,000 seeded vehicles on the free tier isn't viable. Upstash caps at 500K commands/month; a single writer tick against this dataset scale burns through that budget in hours. Run it locally with `npm run seed` and `WRITER_SECRET` set to see the full real-time path.
2. **`regionHint` vs. `resolveCountry()` disagreement** (detailed above under [Global data generation](#global-data-generation)) — a bin's generated plate/model region can disagree with its geo-resolved country label at the margins. Global support is fully implemented — real `d3-geo` point-in-polygon resolution against `world-countries.topo.json`, per-region vehicle model pools, per-region plate formats, land validation — this is a minor edge-case seam at the margins, not a gap in the feature.

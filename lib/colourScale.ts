...
export const HEALTH_LOW = '#EF4444'; // unhealthy
...
export function healthIndex(bin: Pick<BinSummary, 'avg_soh' | 'open_exceptions' | 'vehicle_count'>): number {
  ...
  const exceptionRate =
    bin.vehicle_count > 0 ? bin.open_exceptions / bin.vehicle_count : 0;
...
export function binFill(
  bin: Pick<BinSummary, 'avg_soh' | 'open_exceptions' | 'vehicle_count'>,
  densityNorm: number,
  mode: ViewMode
): FillResult {
  ...
  switch (mode) {
    case 'health':
      return { fill: healthColour(index), fillOpacity: OPACITY_MAX };
    case 'density':
      return { fill: DENSITY_NEUTRAL, fillOpacity: densityOpacity(densityNorm) };
    case 'combined':
    default:
      return { fill: healthColour(index), fillOpacity: densityOpacity(densityNorm) };
  }
}

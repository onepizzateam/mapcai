...
export function CountrySummary() {
  ...
  useEffect(() => { let cancelled = false; fetch('/world-countries.topo.json').then((r) => r.json()).then((topo) => { const geo: any = feature(topo, topo.objects.countries); const out: Record<string, string> = {}; for (const b of bins) { const f = (geo.features ?? [geo]).find((x: any) => geoContains(x, [b.lng, b.lat])); out[b.id] = f?.properties?.name ?? 'Unknown'; } if (!cancelled) setCountries(out); }).catch(() => undefined); return () => { cancelled = true; }; }, [bins]);

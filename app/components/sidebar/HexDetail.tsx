...
export function HexDetail({ hex }: { hex: HexDatum }) {
  ...
  const allBins = useFleetStore((s) => s.bins);
  ...
  const country = hex.bins[0]?.country || approximateCountry(hex.bins[0]?.lat ?? 0, hex.bins[0]?.lng ?? 0);
  const countryBins = useMemo(() => allBins.filter((bin) => (bin.country || approximateCountry(bin.lat, bin.lng)) === country), [allBins, country]);
  const countryAggregate = useMemo(() => weightedAggregate(countryBins), [countryBins]);
  const zones = useMemo(() => zoneRollup(countryBins), [countryBins]);
  const ranked = [...hex.bins].sort((a, b) => (a.avg_soc ?? 0) - (b.avg_soc ?? 0));
  const fleet = allBins.reduce((n, b) => n + b.vehicle_count, 0);
  const countryShare = fleet ? countryAggregate.vehicle_count / fleet * 100 : 0;
  const hexShare = fleet ? aggregate.vehicle_count / fleet * 100 : 0;

  return <div className="flex flex-col gap-5 p-4">
    <div className="flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-wide text-text-muted">Country analytics</p><h2 className="text-base font-semibold text-text-primary">{country}</h2></div><button type="button" onClick={() => { selectHex(null); getFleetState().selectBin(null); }} className="rounded-md border border-border px-2 py-1 text-[11px] text-text-muted">Back to overview</button></div>
    <section className="rounded-lg border border-border bg-bg p-3"><MetricRow label="Open exceptions" value={`${countryAggregate.open_exceptions} open`} hint={`· ${countryAggregate.vehicle_count ? (countryAggregate.open_exceptions / countryAggregate.vehicle_count * 1000).toFixed(1) : '0.0'} per 1k`} /><MetricRow label="Share of fleet" value={`${countryShare.toFixed(1)}%`} hint={`· ${countryAggregate.vehicle_count.toLocaleString()} vehicles`} /></section>
    ...
    <section className="border-t border-border pt-4"><p className="text-[10px] uppercase tracking-wide text-text-muted">Selected hex</p><div className="mt-1 flex items-end justify-between"><div><p className="text-3xl font-semibold tracking-tight text-text-primary">{aggregate.vehicle_count.toLocaleString()}</p><p className="text-xs text-text-muted">vehicles · {hex.bins.length} areas</p></div><span className="rounded-full bg-accent/10 px-2 py-1 text-[10px] font-medium text-accent">{hexShare.toFixed(1)}% of fleet</span></div><div className="mt-3 grid grid-cols-2 gap-2">{[["Vehicles", aggregate.vehicle_count.toLocaleString()], ["Avg SOH", `${aggregate.avg_soh.toFixed(1)}%`], ["Avg range remaining", `${Math.round(aggregate.avg_range_km)} km`], ["Avg SOC", `${aggregate.avg_soc.toFixed(1)}%`], ["Degradation rate", `${aggregate.avg_degradation_rate.toFixed(1)}%/yr`], ["Energy spend today", `₹${aggregate.energy_cost_today_inr.toLocaleString()}`], ["Open exceptions", `${aggregate.open_exceptions} · ${aggregate.vehicle_count ? (aggregate.open_exceptions / aggregate.vehicle_count * 1000).toFixed(1) : '0.0'}/1k`], ["Share of fleet", `${hexShare.toFixed(1)}%`]].map(([label, value]) => <div key={label} className="rounded-md border border-border bg-surface p-2"><p className="text-[10px] text-text-muted">{label}</p><p className="mt-1 font-mono text-sm tabular-nums text-text-primary">{value}</p></div>)}</div></section>
...
function approximateCountry(lat: number, lng: number) { if (lat >= 8 && lat <= 37 && lng >= 68 && lng <= 98) return 'India'; if (lat >= 23 && lat <= 38 && lng >= 60 && lng < 78) return 'Pakistan'; if (lat >= 18 && lat <= 30 && lng >= 97 && lng <= 106) return 'Myanmar'; if (lat >= 20 && lat <= 31 && lng >= 88 && lng < 93) return 'Bangladesh'; return 'Unknown'; }

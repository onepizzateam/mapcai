...
export function HexDetail({ hex }: { hex: HexDatum }) {
  ...
  const country = approximateCountry(hex.bins[0]?.lat ?? 0, hex.bins[0]?.lng ?? 0);
  const countryBins = useMemo(() => allBins.filter((bin) => approximateCountry(bin.lat, bin.lng) === country), [allBins, country]);
